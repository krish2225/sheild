...
const run = async () => {
  ...
  // Check if PM_001 exists, if not create it
  let pm001 = await Machine.findOne({ machineId: 'PM_001' });
  if (!pm001) {
    pm001 = await Machine.create({
      machineId: 'PM_001',
      name: 'High RPM Motor',
    ...
    console.log('Created PM_001 machine');
  }
  ...
  ], { ordered: false }); // Continue even if some machines already exist
